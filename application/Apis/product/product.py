...
from application.models import Products,ProductSchema,Cart,CartSchema,PlacedOrders,PlacedOrdersSchema,FavoriteProducts,Notifications,NotificationsSchema
...
def AddProduct():
    ...
    quality_description = request.form.get('quality_description')
    ...
    product = Products(
        ...
        quality_description=quality_description,sku_code=sku_code,hsn_code=hsn_code,
        ...
        stock_keeping_unit=stock_keeping_unit,variant=variant,tax_tier=tax_tier,order_unit=ordering_unit,
...
def SearchProduct():
    ...
    products_sql = text("SELECT *,(SELECT count(*) FROM favorite_products WHERE  favorite_products.favorite_by="+str(my_id)+" AND favorite_products.favorite_product_id=products.product_id) as is_favorite  FROM products  LEFT JOIN users on users.id=posted_by WHERE product_name LIKE'%"+str(search)+"%' OR tags LIKE'%"+str(search)+"%'  AND posted_by !="+str(my_id))
    query = db.engine.execute(products_sql)
    product_schema = ProductSchema(many=True)
    products = product_schema.dump(query)
